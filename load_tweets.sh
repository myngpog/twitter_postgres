...
for file in $files; do
    ...
    python3 load_tweets.py -- db "postgresql://username:password@host:port/database" --inputs "$file"
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalized
    cat <$file> | sed 's/\\u0000//g' | psql <POSTGRES_DB_URL> -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
